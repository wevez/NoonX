...
public class LegitEntityRotation implements MCHook {
    ...
    private static final List<RandomSample> SAMPLE = Arrays.asList(
...
//        float[] best = RotationUtil.rotation(nearest, eye);
//        float[] x = RotationUtil.rotation(entity.getEyePos(), eye);
//        if (bb.intersects(eye, eye.add(mc.player.getRotationVec(1f).multiply(6)))) {
//            x[0] = RotationUtil.smoothRot(mc.player.getYaw(), x[0], RandomUtil.nextFloat(0, 15));
//            x[1] = RotationUtil.smoothRot(mc.player.getPitch(), x[1], RandomUtil.nextFloat(0, 15) / 2);
//        } else {
//            x[0] = RotationUtil.smoothRot(mc.player.getYaw(), x[0], RandomUtil.nextFloat(3, 30));
//            x[1] = RotationUtil.smoothRot(mc.player.getPitch(), x[1], RandomUtil.nextFloat(3, 30));
//        }
//        float z = RotationUtil.rotation(entity.getEyePos(), eye)[0];
//        float[] za = SAMPLE.stream().min(Comparator.comparingDouble(c -> Math.abs(MathHelper.wrapDegrees(z - c.yaw)))).get().samepl;
//        x[0] += za[0];
//        x[1] += za[1];
//        return x;
        ...
        if (bb.intersects(eye, eye.add(mc.player.getRotationVec(1f).multiply(6)))) {
            if (System.currentTimeMillis() > next) {
                final float[] center = RotationUtil.rotation(entity.getEyePos().add(0, -0.3, 0), eye);
            ...
            float[] za = SAMPLE.stream().min(Comparator.comparingDouble(c -> Math.abs(MathHelper.wrapDegrees(mc.player.getYaw() - c.yaw)))).get().samepl;
            return new float[] {
                    mc.player.getYaw() + aYaw * RandomUtil.nextFloat(1),
                    mc.player.getPitch() + aPitch * RandomUtil.nextFloat(1)
            };
        }
        ...
        z[0] = RotationUtil.smoothRot(mc.player.getYaw(), z[0], RandomUtil.nextFloat(25f, 30));
        //z[1] = RotationUtil.smoothRot(mc.player.getPitch(), z[1], RandomUtil.nextFloat(25f, 30));
